...
- Derive colors from base R palette functions such as colorRampPalette(), heat.colors(), topo.colors(), hcl.colors(), or manually interpolated RGB — choose stops that approximate the requested color palette below. hcl.colors() natively supports "viridis", "plasma", "inferno", and "cividis" by name. There is no built-in "turbo" palette in base R, so when Turbo is requested, build it yourself: colorRampPalette(c("#30123b","#4145ab","#4675ed","#39a2fc","#1bcfd4","#24eca6","#61fc6c","#a4fc3b","#d1e838","#f3c63a","#fe9b2d","#f36315","#d93806","#a91301","#7a0402")).
- A very effective, reliable technique for a single flowing curve (spirograph/rose curve/harmonograph/Lissajous-style superimposed trig functions, parametric attractors, etc.) is to replicate ggplot2's "color mapped continuously along the path" look without ggplot2: compute a long vector of (x, y) points (often tens to hundreds of thousands, from a fine theta/time sequence), then draw it as many short colored segments with a single vectorized call — segments(x[-n], y[-n], x[-1], y[-1], col = adjustcolor(ramp(seq_along(x)[-n]), alpha.f = 0.5..0.8), lwd = 0.3..1) — where ramp is a colorRampPalette(...)/hcl.colors palette function. This is fast (one vectorized C call) even for very large point counts and gives the same continuous-gradient-along-the-curve effect as ggplot2's geom_path(aes(color=t)).
- Strip all chart chrome for this kind of piece: par(mar = c(0,0,0,0)), plot(..., type = "n", axes = FALSE, ann = FALSE, xaxs = "i", yaxs = "i") (or asp = 1 when the shape must stay undistorted), then draw with segments()/lines()/points() on top.
- The script must be deterministic-looking generative art driven by real mathematics: particle systems, strange/chaotic attractors (Lorenz, Rossler, De Jong, Clifford), differential equation integration, flow fields from noise or vector fields, fractals (Mandelbrot/Julia sets, IFS, L-systems), reaction-diffusion systems, recursive geometry, superimposed trigonometric parametric curves (rose curves, harmonographs, Lissajous figures, epicycloids/spirographs), or physical simulations. Do not produce a generic statistical chart, bar plot, or dashboard — the goal is mathematically-grounded visual art, not data visualization.
